...
void i2c_int_handler() {
    ...
    if (msg_ready) {
        ...
        //write gets here
        ToMainHigh_sendmsg(ic_ptr->buflen + 1, MSGT_I2C_WRITE, (void *) ic_ptr->buffer);
    ...
    if (msg_to_send) {
        ...
        //read
        //  ToMainHigh_sendmsg(ic_ptr->buflen, MSGT_I2C_READ, (void *) ic_ptr->buffer);
        readMessages();
...
/**
 * sends reply to master if data is ready, otherwise NACK
 */
void readMessages() {
    ...
    if (sensorFlag == 1) {
        sensorFlag = 0;
        sendReply((void*) sensors, SENSOR_NUMBER + 1);
        //clear values before sending
        sensors[0] = 0x00;
        sensors[1] = 0x00;
        sensors[2] = 0x00;
        sensors[3] = 0x00;
...
void sendNotReady() {
    ...
    start_i2c_slave_reply(6, NACK);

}

/**
 * Send reply as a slave
 * @param sensorData
 * @param num
 */
void sendReply(unsigned char * sensorData, int num) {


    //bottom side
    sensors[0] = sensorData[0];
    // front side
    sensors[1] = sensorData[1];
    //front
    sensors[2] = sensorData[2];
    //sample count
    sensors[3] = sensorData[3];



    if (sendData == 1) {

        //send data out of bounds
        unsigned char outOfRange[6];
        outOfRange[0] = 0x03;
        outOfRange[1] = 0x03;
        outOfRange[2] = sensors[2];
        outOfRange[3] = sensors[1];
        outOfRange[4] = sensors [0];
        outOfRange[5] = ((outOfRange[2]^outOfRange[3]^outOfRange[4]));
        start_i2c_slave_reply(6, outOfRange);
        sendData = 0;

        // if front is less or equal than 36
        //tested
    } else if ((sensors[2] <= 0x36) && frontOut == 0) {
        DEBUG_ON(SENSOR_DATA);
        DEBUG_OFF(SENSOR_DATA);
        //send stop front
        unsigned char stop[6];
        stop[0] = 0x05;
        stop[1] = 0x00;
        stop[2] = 0x00;
        stop[3] = 0x00;
        stop[4] = 0x00;
        stop[5] = 0x00;
        start_i2c_slave_reply(6, stop);
        frontOut = 1;
        sendData = 1;
    }        // if front sensor is more than 40
    else if ((sensors[1] > 0x40) && sideOut == 0) {
        DEBUG_ON(SENSOR_DATA);
        DEBUG_OFF(SENSOR_DATA);
        DEBUG_ON(SENSOR_DATA);
        DEBUG_OFF(SENSOR_DATA);
        //send out of bounds side
        unsigned char stop[6];
        stop[0] = 0x05;
        stop[1] = 0x00;
        stop[2] = 0x00;
         ...
         *
         Local allignment
         */
    else if ((frontOut == 0) && (sideOut == 0) && (((sensors[1] < 0x22) && (sensors[1] < sensors[0] - 2)) || sensors[1] < 0x16) && (sensors[1] < 0x44)) {
        DEBUG_ON(SENSOR_DATA);
        DEBUG_OFF(SENSOR_DATA);
        DEBUG_ON(SENSOR_DATA);
        DEBUG_OFF(SENSOR_DATA);
        DEBUG_ON(SENSOR_DATA);
        DEBUG_OFF(SENSOR_DATA);
        //realign if too close
        unsigned char rightAligned[6];
        rightAligned[0] = 0x08;
        rightAligned[1] = 0x01;
        rightAligned[2] = 0x01;
        rightAligned[3] = 0x00;
        rightAligned[4] = 0x00;
        rightAligned[5] = rightAligned[2];
        start_i2c_slave_reply(6, rightAligned);

    } else if ((frontOut == 0) && (sideOut == 0) && (((sensors[1] > 0x30) && (sensors [1] > sensors[0] + 6)) || sensors[1] > 0x36) && (sensors[1] < 0x44)) {
        ...
        unsigned char leftAligned[6];
        leftAligned[0] = 0x08;
        leftAligned[1] = 0x01;
        leftAligned[2] = 0x00;
        leftAligned[3] = 0x00;
        leftAligned[4] = 0x00;
        leftAligned[5] = leftAligned[2];
        start_i2c_slave_reply(6, leftAligned);
        ...
        sendNotReady();
    //out of bounds
    if (sensors[1] < 0x40 && sensors[0] < 0x40) {
        sideOut = 0;
    }
